                ...
                <form action="{{ route('student.profile.update') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                        ...
                        <div>
                            ...
                            <x-text-input id="class" name="class" type="text" class="block mt-1 w-full" value="{{ old('class', $student->class) }}"/>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <x-text-input id="birth_place" name="birth_place" type="text" class="block mt-1 w-full" value="{{ old('birth_place', $student->birth_place) }}"/>
                        </div>
                        <div>
                            <x-input-label for="birth_date" value="Tanggal Lahir" />
                            <x-text-input id="birth_date" name="birth_date" type="date" class="block mt-1 w-full" value="{{ old('birth_date', $student->birth_date ? \Carbon\Carbon::parse($student->birth_date)->format('Y-m-d') : '') }}"/>
                        </div>
                        <div>
                            <x-input-label for="religion" value="Agama" />
                            <x-text-input id="religion" name="religion" type="text" class="block mt-1 w-full" value="{{ old('religion', $student->religion) }}"/>
                        </div>
                        <div>
                            <x-input-label for="phone" value="No. Handphone / WA" />
                            <x-text-input id="phone" name="phone" type="text" class="block mt-1 w-full" value="{{ old('phone', $student->phone) }}"/>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        <div>
                            ...
                            <x-text-input id="parent_name" name="parent_name" type="text" class="block mt-1 w-full" value="{{ old('parent_name', $student->parent_name) }}"/>
                        </div>
                        <div>
                            <x-input-label for="parent_phone" value="No. Handphone / WA Orang Tua" />
                            <x-text-input id="parent_phone" name="parent_phone" type="text" class="block mt-1 w-full" value="{{ old('parent_phone', $student->parent_phone) }}"/>
                        </div>
                        <div class="md:col-span-2">
                            <x-input-label for="parent_address" value="Alamat Lengkap Orang Tua" />
                            <textarea id="parent_address" name="parent_address" rows="2" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm">{{ old('parent_address', $student->parent_address) }}</textarea>
                        </div>
                    </div>
